...
if __name__ == '__main__':
    ...
    video = cv.VideoCapture('./Videos/data_2/challenge_video.mp4')
    img_num=0
    while video.isOpened():
        grabbed, file = video.read()
        if grabbed==False:
            break
        #file=img2[img_num]
        # 4 points on the original image's lane lines (two from each line)
        source = np.array([[221, 693], [508, 525], [815, 518], [1147, 688]])
        # 4 new points line) for where the image should line up
        dest = np.array([[0, 1200], [0, 0], [300, 0], [300, 1200]])
        if verbose >1:
            cv.line(file, (221, 693), (533, 509), (0, 255, 0), 1)
            cv.line(file, (1147, 688), (777, 507), (0, 255, 0), 1)
        ...
        unwarped = cv.warpPerspective(dst, h, (500, 850))
        ...
        cv.imshow("unwarped", unwarpedGray)
        ...
        ret, BW_lanesLeft = cv.threshold(unwarpedGray, 150, 255, cv.THRESH_BINARY)
